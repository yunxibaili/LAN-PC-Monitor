...
**Purpose:** Finalize a ticket after `/execution/execute-ticket` by transitioning Jira and updating the implementation plan.
...
Please either:
1. Run `/execution/execute-ticket {ticket-key}` first
...
This command can be used standalone (without prior `/execution/execute-ticket`) if the user provides details:

```
/execution/complete-ticket CC-123
```
