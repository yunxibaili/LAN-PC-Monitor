...
- **"手动添加"按钮**：输入 IP + 端口 + 别名 + token。
- **"自动扫描"按钮**：弹出对话框，列出 UDP 心跳发现的在线节点，支持**多选批量添加**。
...
│       ├── node_manager.py      # 节点管理器（摘要列表 + 添加/扫描）
...
**下一步**：依据本文档与《需求增强说明.md》开始编码实现。建议从 P0 骨架起步，先打通"采集节点后台推送 + 副机端本机仪表盘/节点管理 + 主机端集中接收显示 + 本机节点"，再逐层补全采集器与多节点管理。v3.0 时代代码（node_main.py / host_main.py）可作为采集器与 GUI 组件的参考源码，按本架构重组到 `node/`、`client/`、`host/` 包中。
