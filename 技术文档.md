...
- **连接码**：节点端每次启动生成一个短字符串（如 `PCM-8A3B-9F2C`），包含节点地址、端口、token 的加密摘要，显示在节点端控制台窗口或本地通知中。用户在主机/副机端输入该连接码即可自动解析并接入节点（格式见 §23.3）。
...
**连接码格式**：`PCM-XXXX-XXXX`（如 `PCM-8A3B-9F2C`），由节点启动时生成，编码 `ip:port:token` 的加密摘要 + 校验位：
