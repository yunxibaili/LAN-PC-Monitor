...
| 序 | 内容 | 级别 | 理由 |
|----|------|------|------|
| v5.3-1 | **netifaces 替换**（psutil.net_if_addrs / ifaddr） | P2 | CI 已暴露无 cp310+ wheel，是影响开发/部署的工程问题，先清 |
| v5.3-2 | **history.db 路径统一** | P2 | 涉及 storage_service / config / 用户数据目录，需稳定设计 |
| v5.3-3 | **Settings dirty 双模型合并** | P2 | 内部模型整理，无外部依赖 |
| v5.3-4 | **Agent/Host Theme 统一**（common/theme.py 迁移） | P2 | 视觉 + 架构迁移，范围最大，最后做 |
